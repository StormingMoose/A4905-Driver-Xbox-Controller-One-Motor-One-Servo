...
setInterval(() => {
  ...
  if(gamepadIndex !== undefined) {
    ...
    document.body.innerHTML = "  Otto Goes Wild!"; // reset page
 
    let y = myGamepad.axes[0];             // left joystick   - Direction   (servo)  
    let v = myGamepad.axes[1];             //  not used
    let z = myGamepad.axes[2];             //  not used  
    let x = myGamepad.axes[3];             //  right joystick - Speed    (motor)

    if(y<.3 && y >-.3){steer(85); }   //  center dead spot for the steering and speed joystick
    else { if(y < -0.3){steer(85-((y+.3)*(28/.7)*-1)); }      //  joystick steer Left adjusting for dead spot
           else{ if(y > 0.3){steer(85+(y-.3)*(28/.7));}}}  //  joystick right 
  
    if(x<.2 && x >-.2){speed(0,"A");    }     //  center dead spot for the steering and speed joystick
    else { if(x < -0.2){speed((((x+.2)*(255-75)/.8+75*-1)*-1),"B"); }      //  joystick back pressed   ******************
           else{ if(x > 0.2){speed(((x-.2)*(255-125)/.8+125),"A");}}}   //  joystick forward pressed
